...
****************************参数设置函数************************
      ...
      disable_angle_range_config：关闭关节角度限位属性，关机或重启后依然失效。
...
uart = serial.Serial('/dev/cu.usbmodem9286BF920D4B1', uart_baudrate)  # 在 jetson nano（ubuntu） 下控制一体化关节，相应的输入连接的串口
...
def set_id(id_num=0, new_id=0):
    ...
    """
    try:
        write_property(id_num=id_num, property='axis.config.can_node_id', value=new_id)
        if read_property(id_num=id_num, property='axis.config.can_node_id') == new_id:
        ...
        if len(id_list) == len(angle_list):
            if mode == 0:
                ...
                if speed > 0 and param > 0:
                    ...
                    # print(delta_angle) # 打印最大相对角度
                    if delta_angle <= (6 * speed * speed / abs(param)):  # 最大相对角度小于完整加减速所经过的角度
                        t = 2 * cm.sqrt(delta_angle / (6 * abs(param)))  # 等腰三角形轨迹所需要的时间
                    else:
                        t = speed / abs(param) + delta_angle / (6 * speed)  # 经历完整梯形轨迹所需要的时间
                    # print('t=' + str(t)) # 打印完成本次运动所需的时间
                    for i in range(len(id_list)):
...
def set_angles_adaptive(id_list=[0, 0, 0], angle_list=[0, 0, 0], speed_list=[0, 0, 0], torque_list=[0, 0, 0]):
    ...
    try:
        ...
        for i in range(length):
            preset_angle(id_num=id_list[i], angle=angle_list[i], t=abs(speed_list[i]), param=abs(torque_list[i]),
                         mode=1)
        order_num = 0x11
...
# 单个关节阻抗控制
def impedance_control(id_num=0, angle=0, speed=0, tff=0, kp=0, kd=0):
    """单个一体化关节阻抗控制函数。
...
# 多个关节阻抗控制
def impedance_control_multi(id_list=[1, 2, 3], angle_list=[0, 0, 0], speed_list=[0, 0, 0], tff_list=[0, 0, 0],
    ...
    try:
        ...
        for i in range(lenth):
            ...
            if kp_list[i] != 0:
                ...
                # print("angle_list[", i, "]", angle_list[i])
            else:
                print("机器人中关节不允许不间断连续旋转")
                return False
...
def motion_aid(id_num=0, angle=0, speed=0, angle_err=0, speed_err=0, torque=0):
    ...
    Args：
           id_num：一体化关节 ID 编号，如果不知道当前一体化关节 ID，可以用 0 广播，此时总线上有多个一体化关节，则多个一体化关节都会执行该操作。
            angle: 目标角度（°），即助力目标角度，该值减去关节当前角度即为助力行程。
    ...
    try:
        ...
        set_speed_limit(id_num=id_num, speed_limit=speed)
    except Exception as e:
...
# 多个关节运动助力
def motion_aid_multi(id_list=[1, 2, 3], angle_list=[0, 0, 0], speed_list=[0, 0, 0], angle_err_list=[1, 1, 1],
    ...
    try:
        ...
        if len(angle_err_list) != length or len(speed_list) != length or len(speed_err_list) != length or len(
            ...
            print("关节数量与参数数量不一致，请检查")
        for i in range(length):
            # preset_angle(id_num=id_list[i], angle=angle_err_list[i], t=abs(speed_err_list[i]), param=abs(torque_list[i]),
            #              mode=1)
            data = format_data(
                [int(angle_list[i] / factor), int(angle_err_list[i] / factor), int(speed_err_list[i] / factor),
                 int(torque_list[i] / factor)], 's16 u16 u16 s16',
                'encode')
            send_command(id_num=id_list[i], cmd=0x06, data=data, rtr=0)  # 需要用标准帧（数据帧）进行发送，不能用远程帧
        order_num = 0x11
...
def position_done(id_num=0):
    ...
    try:
        while kk == 0:
            kk = read_property(id_num, property='axis.controller.trajectory_done')
            # print(id_num)
    except Exception as e:
...
def get_id():
    ...
    """
    try:
        id = read_property(id_num=0, property='axis.config.can_node_id')
        print(id)
        return id
    except Exception as e:
...
def get_ids():
    ...
    try:
        ...
        property = 'axis.config.can_node_id'
        address = property
...
def get_angle(id_num=0):
    ...
    """
    try:
        # angle = read_property(id_num=id_num, property='axis.output_shaft.pos_estimate')
        # return angle
        return read_property(id_num=id_num, property='axis.output_shaft.pos_estimate')
        # if READ_FLAG == 1:
        #     return angle
        # else:
        #     return
...
def get_speed(id_num=0):
    ...
    """
    try:
        # speed = read_property(id_num=id_num, property='axis.output_shaft.vel_estimate')
        # return speed
        return read_property(id_num=id_num, property='axis.output_shaft.vel_estimate')
        # if READ_FLAG == 1:
        #     return speed
        # else:
        #     return
...
def get_state(id_num=0):
    ...
    try:
        return [read_property(id_num=id_num, property='axis.output_shaft.pos_estimate'),
                read_property(id_num=id_num, property='axis.output_shaft.vel_estimate')]
    except Exception as e:
...
def get_torque(id_num=0):
    ...
    """
    try:
        # torque = read_property(id_num=id_num, property='axis.output_shaft.torque_estimate')
        # return torque
        return read_property(id_num=id_num, property='axis.output_shaft.torque_estimate')
        # if READ_FLAG == 1:
        #     return torque
        # else:
        #     return
...
def get_vol_cur(id_num=0):
    ...
    try:
        ...
        vol_cur[1] = read_property(id_num=id_num, property='axis.motor.current_control.Iq_measured')
...
def enable_angle_speed_torque_state(id_num=0):
    ...
    try:
        write_property(id_num, 'can.config.enable_state_feedback', 1)
        return True
    except Exception as e:
...
def set_state_feedback_rate_ms(id_num=0, n_ms=2):
    ...
    try:
        for i in range(5):
            write_property(id_num, 'axis.config.state_feedback_rate_ms', n_ms)
        return True
    except Exception as e:
...
angle_speed_torque：[angle, speed, torque]
    ...
    try:
        ...
        if READ_FLAG == 1:
            for i in range(n):
                ...
                if id_num == (cdata[1] * 256 + cdata[2] - 1) >> 5:
                    ...
                    return [round(angle_speed_torque[0], 3), angle_speed_torque[1] * 0.01, angle_speed_torque[2] * 0.01]
        ...
        # sys.exit()
        # if id_is_or_no == 0:
        #     print("state_data 函数中 ID 号有误")
        #     sys.exit()

        # cdata = uart_to_can_ID(data=udata)
        # if id_num == (cdata[1] * 256 + cdata[2] - 1) >> 5:
        #     angle_speed_torque = format_data(data=cdata[3:], format='f s16 s16', type='decode')
...
def disable_angle_speed_torque_state(id_num=0):
    ...
    try:
        for i in range(5):
            write_property(id_num, 'can.config.enable_state_feedback', 0)
        read_data(id_num)
        return True
    except Exception as e:
...
def get_pid(id_num=0):
    ...
    """
    try:
        P = read_property(id_num=id_num, property='axis.controller.config.pos_gain')
        I = read_property(id_num=id_num, property='axis.controller.config.vel_integrator_gain')
        D = read_property(id_num=id_num, property='axis.controller.config.vel_gain')
        print('位置增益 P：%s' % P)
        print('积分增益 I：%s' % I)
        print('转速增益 D：%s' % D)
    except Exception as e:
...
def read_property(id_num=0, property=''):
    ...
    Args：
          id_num：一体化关节 ID 编号，如果不知道当前一体化关节 ID 编号，可以用 0广播。
                  但此时如果总线上有多个一体化关节，会造成总线通信干扰，不可使用 0 广播。
        property：需要设置的属性参数名称，例如 "vbus_voltage"，"axis.config.can_node_id"等，
...
def set_zero_position(id_num=0):
    ...
    设置当前角度为一体化关节输出轴零点，设置完后当前角度为 0 度。
    ...
    try:
        ...
        # cdata = receive_data()
        # if READ_FLAG == 1:
        #     property = format_data(data=cdata, format='u16 u16 s16', type='decode')
        #     if len(property) > 0:
        #         return property[-1]
        #     else:
        #         return
    ...
    try:
        ...
        if READ_FLAG == 1:
            if angle >= angle_min and angle <= angle_max:
                write_property(id_num=id_num, property='axis.output_shaft.angle_min', value=angle_min)
                write_property(id_num=id_num, property='axis.output_shaft.angle_max', value=angle_max)
                write_property(id_num=id_num, property='axis.output_shaft.enable_angle_limit', value=1)
            else:
...
def disable_angle_range(id_num=0):
    ...
    """
    try:
        write_property(id_num=id_num, property='axis.output_shaft.enable_angle_limit', value=0)
        bol = read_property(id_num=id_num, property='axis.output_shaft.enable_angle_limit')
        if bol == 0:
...
def set_angle_range_config(id_num=0, angle_min=-180, angle_max=180):
    ...
    try:
        ...
        if READ_FLAG == 1:
            if angle >= angle_min and angle <= angle_max:
                write_property(id_num=id_num, property='axis.config.extra_setting.angle_min',
                               value=angle_min)
                write_property(id_num=id_num, property='axis.config.extra_setting.angle_max',
                               value=angle_max)
                write_property(id_num=id_num, property='axis.config.extra_setting.enable_angle_limit',
                               value=1)
                save_config(id_num=id_num)
            else:
                print("一体化关节输出轴当前角度不在" + str(
...
def disable_angle_range_config(id_num=0):
    ...
    """
    try:
        write_property(id_num=id_num, property='axis.config.extra_setting.enable_angle_limit', value=0)
        bol = read_property(id_num=id_num, property='axis.config.extra_setting.enable_angle_limit')
        save_config(id_num=id_num)
        if bol == 0:
            print("取消角度限位属性成功")
        else:
...
def set_pid(id_num=0, P=20, I=25, D=20):
    ...
    try:
        if P <= 0 or D <= 0 or I <= 0:
            ...
            return False
        write_property(id_num=id_num, property='axis.controller.config.pos_gain', value=P)
        write_property(id_num=id_num, property='axis.controller.config.vel_integrator_gain', value=I)
        write_property(id_num=id_num, property='axis.controller.config.vel_gain', value=D)
    except Exception as e:
...
# # 使能 CAN 通信终端电阻
# def set_can_120r_enable(id_num=0, enable=1):
#     """CAN 通信匹配电阻使能函数
#
#     设置一体化关节内部的 CAN 通信终端匹配电阻（120欧）是否使能，一般情况下，只需将离上位机 CAN 接口最远的那个一体化关节使能即可。
#     （关机后依然保存）
#
#     Args：
#         id_num：一体化关节ID号,如果不知道当前一体化关节编号，可以用0广播。
#         enable：使能标志位，
#                 enable=1，表示使能120欧终端电阻；
#                 enable=0，表示不使能120欧终端电阻。
#
#     Returns：
#          True：设置成功。
#         False：设置失败。
#
#     Raises：
#         ---error in set_can_120r_enable---
#
#     """
#     try:
#         write_property(id_num=id_num, property='axis.config.extra_setting.enable_120r', value=enable)
#         save_config(id_num=id_num)
#     except Exception as e:
#         print("---error in set_can_120r_enable---：", e)
#         return False
#     return True
...
def dump_error(id_num=0):
    ...
    读取一体化关节错误信息编码，如果错误编码为 0，表示无异常。如果错误编码不为 0，则表示存在故障。
    一般情况下使用 reboot 函数重启后可清除故障，如重启未清除故障请咨询技术支持。

    Args：
        id_num：一体化关节 ID 编号，如果不知道当前一体化关节 ID 编号，可以用 0 广播，
                但此时如果总线上有多个一体化关节，会造成总线通信干扰，不可使用 0 广播。
    ...
    """
    try:
        axis_error = read_property(id_num=id_num, property='axis.error')
        motor_error = read_property(id_num=id_num, property='axis.motor.error')
        encoder_error = read_property(id_num=id_num, property='axis.encoder.error')
        controller_error = read_property(id_num=id_num, property='axis.controller.error')
        can_error = read_property(id_num=id_num, property='can.error')
        fet_thermistor = read_property(id_num=id_num, property='axis.fet_thermistor.error')
        motor_thermistor = read_property(id_num=id_num, property='axis.motor_thermistor.error')
...
def erase_config(id_num=0):
    """擦除配置参数。适用于意外情况下驱动板损坏，更换驱动板后需进行参数初始化和电机标定的场合，一般情况下无需使用。

    擦除配置参数。该函数执行过程中会将关节重新标定，因此执行该函数时请勿将关节挂载。
    注意：
    ...
    Raises:
        "---error in erase_config---"

    """
    try:
        data = format_data([id_num], 'u32', 'encode')
        send_command(id_num=id_num, cmd=0x0F, data=data, rtr=0)  # 需要用标准帧（数据帧）进行发送，不能用远程帧
        # time.sleep(1)
        # save_config(id_num=id_num)
    except Exception as e:
        print("---error in erase_config---：", e)
        return False
    return True
...
# USB转CAN模块包模式：串行帧->CAN报文，同时计算CAN节点ID
...
# CAN实时状态接受函数，包含CAN节点ID
# def angle_speed_torque_state(id_num=0, n=1):
#     udata = read_data_state(n) # can 报文一共16个字节
#     # print("udata: ", udata)
#     if READ_FLAG == 1:
#         # id_is_or_no = 0
#         for i in range(n):
...
def preset_speed(id_num=0, speed=10, param=0, mode=1):
    ...
    Args:
        ...
        mode:   控制模式选择
                ...
                mode!=0,转速爬升控制模式，一体化关节将按照目标加转速axis.controller.config_.vel_ramp_rate变化到speed。

    Note:
        在转速爬升模式下，如果目标加转速axis.controller.config_.vel_ramp_rate设置为0，则一体化关节转速将保持当前值不变。
...
def preset_torque(id_num=0, torque=0.1, param=0, mode=1):
    ...
    Note;
        如果一体化关节转速超过您设置的 vel_limit ，一体化关节输出的力矩将会减小。
        可以设置 axis.controller.config.enable_current_mode_vel_limit = False 来禁止力矩减小。
