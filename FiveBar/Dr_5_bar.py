...
class robot(am.arm):
    ...
    Attributes:
        ...
        max_speed: 机器人最大运动速度，一般取各个关节一体化关节中转速最低的那个；
    ...
    Methods:
        ...
        detect_position：根据回读的一体化关节角度反馈机器人当前位姿函数
        ###############辅助功能函数####################
        save_position: 基于机器人当前末端位姿求解各关节模型角度，并保存position_list中；
        model_to_servo: 将position_list中保存的机器人关节模型角度转换成一体化关节实际角度；
        do_motion: 动作执行函数，让机器人依次执行之前保存的动作；
        range_init: 机器人关节转动范围设置函数，设置的是模型角度范围；
        add_position: 将当前机器人姿态保存到position_list中
        read_joints：读取机器人所有关节一体化关节当前角度；
        read_joints_p_v：读取机器人所有关节一体化关节当前角度和转速
    ...
    position_list_temp = []
    ID_list = [1, 2]
    P1_list = [1, 1]  # 对应关节一体化关节输出轴与模型中关节z轴方向一致则为1，不一致则为-1
    P2_list = [0, 0] # 一体化关节在机器人初始位置安装时的实际角度，本机器人关节皆为在零位安装
    MAX_list = [360, 180]
    MIN_list = [0, -180]
    ...
    # 机器人初始化函数
    def __init__(self, MAX_list_temp=[], MIN_list_temp=[], L_temp = [150, 225, 150, 225, 150]):
        """机器人对象初始化函数，初始化机器人对象的属性参数

        Args:
            MAX_list_temp: 机器人关节模型角度最大值组成的列表
            MIN_list_temp: 机器人关节模型角度最小值组成的列表
            l_temp：机器人尺寸参数：[l1 l2 l3 l4 l5]，详见库函数说明
        Returns:
            无
        Raises:
            无
        ...
        servo_angle_list = self.read_joints() # 读取机器人关节一体化关节实际角度
        if servo_angle_list != False:
            ...
            self.c_angle_list = self.init_model_angle[:] # 若关节一体化关节读取失败则关节模型角度取默认值
        self.theta = [DEG_RAD * i for i in self.c_angle_list].copy() # 初始化关节模型角度（弧度制）
        self.forward_kinematics_position(self.c_angle_list) # 根据初始化的关节模型角度计算初始的末端位置tip_x_y和姿态phi
        position = [self.theta[0] / math.pi * 180, self.theta[1] / math.pi * 180]
        self.position_list.append(position)
        position_list_servo_temp = self.read_joints() # 再读取一次关节一体化关节角度，以进行后面的操作
    ...
    # 一、机器人位置和姿态控制函数
    def set_position(self, tip_x_y=[0, 0], speed=1.0, acceleration=10, ud=0):
        """控制机器人运动到指定位置和姿态

        Args:
            tip_x_y: 机器人末端位置坐标组成的列表[x, y]
            speed: 当前姿态被执行时转速最快关节的转动速度
            acceleration：驱动关节加速度大小
        ...
        self.clear_position()
        if tip_x_y[1] < 0: # 输入的 y 坐标小于零时，选择对称构型
            ud = 1
        if self.save_position(tip_x_y=tip_x_y, ud=ud):  # 运动学逆解并将结果保存进 position_list
            self.do_motion(speed=speed, acceleration=acceleration)  # 执行动作
        else:
            self.tip_x_y = tip_x_y_bk.copy()  # 若运动学逆解失败，则恢复此前的末端位置坐标

    def set_relative_position(self, tip_x_y=[0, 0], speed=1.0, acceleration=10, ud=0):
        ...
        self.clear_position()
        if self.save_position(tip_x_y=tip_x_y, ud=ud):  # 运动学逆解，并将结果存进 position_list
            self.do_motion(speed=speed, acceleration=acceleration)  # 执行动作
        else:
            self.tip_x_y = tip_x_y_bk.copy()  # 若运动学逆解失败，则恢复此前的末端姿态角
    #
    # 二、机器人轨迹控制函数
    def set_positions(self, tip_x_ys=[], t=1):
        """控制机器人末端按顺序连续运动到多个指定位置和姿态

        Args:
            tip_x_ys: 机器人末端连续多个坐标组成的列表[x, y]
            t: 运动执行的大致时间
        Returns:
            无
        Raises:
        ...
        for i in range(n): # 将输入的坐标值和姿态角全部带入逆解函数求解出关节模型角度
            ...
            if not self.save_position(tip_x_y=tip_x_y, ud=0):  # 运动学逆解，并将求解结果存进 position_list
                self.tip_x_y = tip_x_y_bk.copy()  # 如果逆解失败则取此前记录的末端坐标值
        self.position_list_servo = []  # position_list_servo 在此置空，用于存放计算得到的关节一体化关节实际角度列表
        for j in range(len(self.position_list)):
            self.position_list_servo.append(self.model_to_servo(model_angle_list=self.position_list[j])) # 将关节模型角度转换成关节一体化关节实际角度并添加进 position_list_servo
        self.set_position(tip_x_y=tip_x_ys[0], speed=10)
        self.position_done()
    ...
    #
    def set_positions_curve_pre(self, tip_x_ys=[]):
        """预先计算机器人按顺序连续运动到多个指定位置和姿态所对应的关节一体化关节角度
    ...
    def set_positions_curve_start_point(self, speed=10):
        """控制机器人运动到轨迹中的首个位置和姿态, 用于set_arm_positions_cure_pre之后
        ...
        self.set_position(tip_x_y=self.tip_x_y_start, speed=speed) # 控制机器人运动到轨迹中的首个位置和姿态
        self.position_done() # 监控并等待动作执行结束

    def set_positions_curve_do(self, t=1):
        """控制机器人按顺序连续运动到多个指定位置和姿态, 与set_arm_positions_cure_pre连用
        ...
        if max(DETA_angle_list) > 1.0:  # 如果轨迹起点离当前位置比较远，则自动调用set_positions_curve_start_point(speed=10)，否则无需调用
            self.set_positions_curve_start_point(speed=10) # 防止用户忘记调用 set_positions_curve_start_point(speed=10)
        n = len(self.position_list_servo)
        if t >= n * self.bit_time: # 轨迹要求的执行时间大于系统发送 n 个动作指令（所有轨迹点）所需的时间
            print('t >= n * bit_time')  # 提示
            bit_wideth = n / t / 2 # 指令发送带宽的一半
    ...
    def add_position(self, t=0):
        ...
        servo_list = self.read_joints() # 读取关节一体化关节角度
        if servo_list != False:
            self.position_list.append(self.servo_to_model(servo_angle_list=servo_list)) # 将关节一体化关节角度转换成关节模型角度，并添加进 position_list 中
    ...
    #
    # 四、机器人关节角度控制函数
    def set_joints(self, angle_list=[90, 90], speed=1.0, acceleration=10):
        """控制机器人驱动关节运动到指定模型角度

        Args:
            angle_list: 机器人六个关节模型角度组成的列表[joint1, joint2]
            speed: 当前姿态被执行时转速最快关节的转动速度
        Returns:
            无
        Raises:
        ...
        if len(angle_list) == len(self.ID_list):
            position = angle_list[:]
            self.theta = [math.pi / 180 * i for i in angle_list] # 更新内存中的关节模型角度
    ...
    def detect_position(self):
        ...
        servo_angle_list = self.read_joints() # 读取各关节一体化关节角度（用于查询未知姿态，因此读取关节角度）
        model_angle_list = self.servo_to_model(servo_angle_list=servo_angle_list) # 将关节一体化关节角度转换为关节模型角度
        self.forward_kinematics_position(model_angle_list) # 调用运动学正解函数
        print("当前机器人末端x坐标为: " + str(self.tip_x_y[0])+ "; y坐标为： " + str(self.tip_x_y[1]))
    ...
    # 七、功能辅助函数
    def save_position(self,  tip_x_y=[0, 0], ud=0):
        """求解并保存当前机器人姿态下各个关节模型角度，并将角度制的角度列表添加进 position_list

        Args:
            tip_x_y：机器人末端位置坐标
            ud_temp: (up/down)用来选择逆解中的第几种解：
        ...
        theta_bk = self.theta.copy() # 记录当前驱动关节角度
        if not self.inverse_kinematics(tip_x_y=tip_x_y, ud=ud):  # 在对应支链中进行运动学逆解
        ...
        for i in range(len(self.ID_list)):
            ...
            if servo != False:
                ...
                if servo != False:
                    ...
                    flag.append(self.ID_list[i]) # 如果持续读取失败则记录读取失败的关节一体化关节编号
    ...
    def servo_to_model(self, servo_angle_list=[]):
        ...
        if servo_angle_list != False:
            ...
            for i in range(len(servo_angle_list)):
                model_angle_list[i] = round((servo_angle_list[i] - self.P2_list[i]) / self.P1_list[i], 1) # 通过关节模型角度与关节一体化关节角度之间的关系进行转换
            return model_angle_list
        else:
            return False

    def model_to_servo(self, model_angle_list=[]):
        """将机器人关节模型角度转换成关节一体化关节角度。
        ...
        for i in range(len(model_angle_list)):
            servo_angle_list[i] = self.P1_list[i] * servo_angle_list[i] + self.P2_list[i] # 通过关节模型角度与关节一体化关节角度之间的关系进行转换
        return servo_angle_list

    # 将所有关节设置成待机模式
    def free(self):
        """将机器人所有关节一体化关节设置成待机模式，以便节省能源。若要恢复使用必须先调用 lock()函数。
        Args:
            无；
        Returns:
            无；
    ...
    def position_done(self):
        ...
        dr.positions_done(id_list=self.ID_list)
    ...
    def read_property(self, joint_num=1, property=''):
        ...
        if joint_num < 1 or joint_num > 2:
            print("请输入正确的关节编号：1~2")
            return False
        else:
