...
class robot(am.arm):
    """3-RRR并联机器人类（继承arm类），专注于功能实现

    Attributes:
        c_angle_list: 机器人所有关节当前模型角度（角度制）组成的列表；
        pose_list: 用来存放机器人动作序列的列表；
        ID_list: 机器人关节所对应编号组成的列表；
        P1_list: 机器人关节模型角度与一体化关节机实际角度转换的第一个系数的列表；两者的变换关系为：舵机角度=P1*模型角度+P2（模型关节轴线与一体化关节转动轴线同向为1，反向为-1）
        P2_list: 机器人关节模型角度与一体化关节实际角度转化的第二个系数的列表；两者的变换关系为：舵机角度=P1*模型角度+P2（一体化关节处于模型初始位置安装时对应的实际角度）
    ...
    Methods:
        __init__: 对象初始化函数；
        ###############运动控制函数######################
        set_pose：控制机器人末端运动到指定位置和姿态函数
        set_position: 控制机器人末端运动到指定位置函数（不改变姿态角）
        set_orientation：控制机器人末端运动到相对于机器人本体指定姿态角函数（不改变末端位置）
        set_relative_pose：控制机器人相对当前位姿运动一定距离和角度函数
        set_relative_position：控制机器人相对当前位姿运动一定距离函数（不改变姿态角）
        set_relative_orientation：控制机器人相对当前位姿运动一定角度函数（不改变末端位置）
        set_poses：控制机器人连续运动多个位置和姿态函数，即执行一定轨迹
        set_poses_curve_pre：预先设置机器人末端运动轨迹函数
        set_poses_curve_start_point：控制机器人运动到预设轨迹起点的函数，需与在set_arm_poses_curve_pre之后使用
        set_poses_curve_do：控制机器人执行预设轨迹函数
        set_joints：控制机器人各个关节运动到指定模型角度函数
        ###############数据回读函数####################
        show_pose：显示当前末端位置和姿态，用在已经执行过动作之后
        ###############辅助功能函数####################
        save_pose: 基于机器人当前末端位姿求解各关节模型角度，并保存pose_list中；
        model_to_servo: 将pose_list中保存的机器人关节模型角度转换成一体化关节实际角度；
        do_motion: 动作执行函数，让机器人依次执行之前保存的动作；
        range_init: 机器人关节转动范围设置函数，设置的是模型角度范围；
        add_pose: 将当前机器人姿态保存到pose_list中
        ...
        pose_done：检查并等待机器人运动到指定位置
    """
    c_angle_list = []
    seta = [] # 内存中驱动关节模型角度
    pose_list = [] # 驱动关节角度列表组成的列表
    servo_angle_list_list = [] # 一体化关节角度列表组成的列表
    pose_list_servo = [] # 一体化关节角度列表
    ID_list = [1, 2, 3] # 一体化关节 ID 号列表
    P1_list = [1, 1, 1]  # 对应一体化关节输出轴与模型中关节z轴方向一致则为1，不一致则为-1
    P2_list = [0, 0, 0] # 一体化关节在机器人初始位置安装时的实际角度，本机器人关节皆为在零位安装
    MAX_list = [160, 180, 160] # 关节最大角度组成的列表
    MIN_list = [-160, -40, -160] # 关节最小角度组成的列表
    init_model_angle = [0, 0, 0] # 关节初始角度
    max_speed = 30 # 一体化关节最大转速
    tutorial_t = 1 # 位姿示教编程中单个位姿停留时间
    tutorial_t_list = [0] # 每个位姿的停留时间组成的列表
    bit_time = 0 # 系统发送一次运动指令所需的时间
    torque_limits = [] # 机器人各一体化关节最大限制力矩组成的列表
    pitch =0 # 当末端z轴方向平行与xy平面时，机器人的pitch角
    tip_x_y_start = [0, 0] # 轨迹规划起始点位置
    phi_start = 0 # 轨迹规划起始点姿态角

    # 机器人初始化函数
    def __init__(self, MAX_list_temp=[], MIN_list_temp=[], L_temp=[150, 150, 115.25, 184.75]):
        """机器人对象初始化函数，初始化机器人对象的属性参数

        Args:
            MAX_list_temp: 机器人关节模型角度最大值组成的列表
            MIN_list_temp: 机器人关节模型角度最小值组成的列表
            l_temp：机器人尺寸参数：[l1, l2, l3, R]
        Returns:
            无
        Raises:
            无
        """
        self.ARM_NUMBER = 3  # 3条支链
        am.arm.__init__(self, l=L_temp) # 将机器人（运动模型）对象初始化
        ...
        if servo_angle_list != False:
            self.c_angle_list = self.servo_to_model(servo_angle_list=servo_angle_list) # 根据读取到的一体化关节角度初始化关节模型角度（角度制）
        else:
            self.c_angle_list = self.init_model_angle[:] # 若一体化关节读取失败则关节模型角度取默认初始值
        self.seta = [DEG_RAD * i for i in self.c_angle_list].copy() # 初始化关节模型角度（弧度制）
        # self.forward_kinematics_pose(self.c_angle_list) # 根据初始化的关节模型角度计算初始的末端位置tip_x_y和姿态phi（3-RRR并联机器人暂无运动学正解）
        pose_list_servo_temp = self.read_joints() # 再读取一次一体化关节角度，以进行后面的操作
        dr.set_angles(self.ID_list, pose_list_servo_temp, 20, 20, 0)
        start = time.time()
    ...
    # 一、机器人位置和姿态控制函数
    def set_pose(self, tip_x_y=[0, 0], phi=0, speed=10, acceleration=10, ud=1):
        """控制机器人运动到指定位置和姿态

        Args:
            tip_x_y: 机器人末端位置坐标组成的列表[x, y]
            phi： 机器人末端姿态角组成的列表
            speed: 当前姿态被执行时转速最快关节的转动速度
            acceleration：关节加速度大小
            ud：(up/down)用来选择支链运动学逆解中的何种解：
        ...
        Raises:
            无
        """
        tip_x_y_bk = self.tip_x_y.copy()  # 记录此前末端位置坐标，此处不能直接使用 tip_x_y_bk = self.tip_x_y 否则 tip_x_y_bk 会始终随着 self.tip_x_y 变化
        phi_bk = self.phi  # 记录此前末端姿态角
        self.tip_x_y = tip_x_y.copy()  # 将目标位置赋值给对象位置属性 tip_x_y
        self.phi = phi  # 将目标位置赋值给对象姿态属性 phi，放在内存中
        self.clear_pose()
        if self.save_pose(tip_x_y=tip_x_y, phi=phi, ud=ud):  # 运动学逆解并将结果保存进 pose_list
            # return self.theta
            self.do_motion(speed=speed, acceleration=acceleration)  # 执行动作
        else:
            self.tip_x_y = tip_x_y_bk.copy()  # 若运动学逆解失败，则恢复此前的末端位置坐标
            self.phi = phi_bk  # 若运动学逆解失败，则恢复此前的末端姿态角

    def set_position(self, tip_x_y=[0, 0], speed=1.0, acceleration=10, ud=1):
        """控制机器人末端运动到指定位置，不改变机器人末端角姿态

        Args:
            tip_x_y: 机器人末端坐标组成的列表[x, y]
            speed: 当前姿态被执行时转速最快关节的转动速度
            acceleration：关节加速度大小
            ud：(up/down)用来选择支链运动学逆解中的何种解：
                ud = 0 时(默认值)，机器人第三个关节在末端点上方,对应课程中的alpha构型；
    ...
    def set_orientation(self, phi=0, speed=1.0, acceleration=10, ud=1):
        """控制机器人末端运动到指定位置，不改变机器人末端角姿态
        ...
        Raises:
            无
        """
        phi_bk = self.phi  # 记录此前末端位置坐标，此处不能直接使用 tip_x_y_bk = self.tip_x_y 否则 tip_x_y_bk 会始终随着 self.tip_x_y 变化
        self.phi = phi  # 将目标位置赋值给对象位置属性 phi
        tip_x_y = self.tip_x_y # 取用内存中的 tip_x_y
        self.clear_pose()
        if self.save_pose(tip_x_y=tip_x_y, phi=phi, ud=ud):  # 运动学逆解并将结果保存进 pose_list
            self.do_motion(speed=speed, acceleration=acceleration)  # 执行动作
        else:
            self.phi = phi_bk  # 若运动学逆解失败，则恢复此前的末端位置坐标
    
    def set_relative_pose(self, tip_x_y=[0, 0], phi=0, speed=1.0, acceleration=10, ud=1):
        """控制机器人运动到相对当前的指定位置和姿态

        Args:
            tip_x_y: 机器人末端相对坐标组成的列表 [x, y]
            phi： 机器人末端相对姿态角
            speed: 当前姿态被执行时转速最快关节的转动速度
            acceleration：关节加速度大小
            ud：(up/down)用来选择支链运动学逆解中的何种解：
        ...
        Raises:
            无
        """
        tip_x_y_bk = self.tip_x_y.copy()  # 记录此前末端坐标，此处不能直接使用 tip_x_y_bk = self.tip_x_y 否则 tip_x_y_bk 会始终随着 self.tip_x_y 变化
        phi_bk = self.phi  # 记录此前末端姿态角
        for i in range(len(tip_x_y)):
            self.tip_x_y[i] = self.tip_x_y[i] + tip_x_y[i]  # 更新内存中的位置坐标
        self.phi = self.phi + phi  # 更新内存中的姿态角
        phi = self.phi # 将更新后的末端姿态角赋值给phi
        tip_x_y = self.tip_x_y.copy()  # 将更新后的末端坐标赋值给tip_x_y
        self.clear_pose()
        if self.save_pose(tip_x_y=tip_x_y, phi=phi, ud=ud):  # 运动学逆解，并将结果存进 pose_list
            self.do_motion(speed=speed, acceleration=acceleration)  # 执行动作
        else:
            self.tip_x_y = tip_x_y_bk.copy()  # 若运动学逆解失败，则恢复此前的末端位置坐标
            self.phi = phi_bk  # 若运动学逆解失败，则恢复此前的末端姿态角

    def set_relative_position(self, tip_x_y=[0, 0], speed=1.0, acceleration=10, ud=1):
        """控制机器人运动到相对当前位置的指定位置，不改变机器人角姿态

        Args:
            tip_x_y: 机器人末端相对部坐标组成的列表[x, y]
            speed: 当前姿态被执行时转速最快关节的转动速度
            acceleration：关节加速度大小
            ud：(up/down)用来选择支链运动学逆解中的何种解：
                ud = 0 时(默认值)，机器人第三个关节在末端点上方,对应课程中的alpha构型；
        ...
        tip_x_y_bk = self.tip_x_y.copy()  # 记录此前末端坐标，此处不能直接使用 tip_x_y_bk = self.tip_x_y 否则 tip_x_y_bk 会始终随着 self.tip_x_y 变化
        for i in range(len(tip_x_y)):
            self.tip_x_y[i] = self.tip_x_y[i] + tip_x_y[i]  # 机器人末端坐标值加上相对坐标值
        phi = self.phi
        tip_x_y = self.tip_x_y.copy() # 将更新后的末端坐标赋值给tip_x_y
        self.clear_pose()
        if self.save_pose(tip_x_y=tip_x_y, phi=phi, ud=ud):  # 运动学逆解，并将结果存进 pose_list
            ...
            self.tip_x_y = tip_x_y_bk.copy()  # 若运动学逆解失败，则恢复此前的末端姿态角

    def set_relative_orientation(self, phi=0, speed=1.0, acceleration=10, ud=1):
        """控制机器人运动到相对于当前姿态的指定姿态，不改变末端位置
        ...
        """
        phi_bk = self.phi # 记录此前末端姿态角
        self.phi = self.phi + phi  # 机器人末端姿态角加上相对姿态角
        tip_x_y = self.tip_x_y.copy()
        phi = self.phi # 将更新后的末端轴姿态赋值给phi
        self.clear_pose()
        if self.save_pose(tip_x_y=tip_x_y, phi=phi, ud=ud): # 运动学逆解，并将结果存进 pose_list
            self.do_motion(speed=speed, acceleration=acceleration) # 执行动作
        else:
    ...
    #
    # 二、机器人轨迹控制函数
    def set_poses(self, tip_x_ys=[], phis=[], t=1):
        """控制机器人末端按顺序连续运动到多个指定位置和姿态

        Args:
            tip_x_ys: 机器人末端连续多个坐标组成的列表[[x1, y1], [x2, y2], [x3, y3], ...]
            phi： 机器人末端连续多个姿态角组成的列表[phi1, phi2, phi3, ...]
            t: 运动执行的大致时间
        Returns:
            无
        Raises:
        ...
        if len(tip_x_ys) > len(phis):
            for ii in range(len(tip_x_ys) - len(phis)):
                phis.append(phis[len(phis) -1]) # 如果输入的姿态角数目少于坐标值数目，则缺少的姿态角用最后一组姿态角补足
        if len(phis) > len(tip_x_ys):
            for ii in range(len(phis) - len(tip_x_ys)):
                tip_x_ys.append(tip_x_ys[len(tip_x_ys) -1]) # 如果输入的坐标值数目少于坐标值数目，则缺少的坐标值用最后一组坐标值补足
        n = len(tip_x_ys)
        self.clear_pose()
        for i in range(n): # 将输入的坐标值和姿态角全部带入逆解函数求解出关节模型角度
            tip_x_y = tip_x_ys[i].copy() # 将轨迹点赋值给 tip_x_y
            phi = phis[i]  # 将轨迹点赋值给 phi
            tip_x_y_bk = self.tip_x_y.copy()  # 记录此前末端坐标值，此处不能直接使用 tip_x_y_bk = self.tip_x_y 否则 tip_x_y_bk 会始终随着 self.tip_x_y 变化
            phi_bk = self.phi  # 记录此前末端姿态角
            self.tip_x_y = tip_x_y.copy()  # 记录此后末端坐标值
            self.phi = phi  # 记录此后末端姿态角
            if not self.save_pose(tip_x_y=tip_x_y, phi=phi, ud=1):  # 运动学逆解，并将求解结果存进 pose_list
                self.tip_x_y = tip_x_y_bk.copy()  # 如果逆解失败则取此前记录的末端坐标值
                self.phi = phi_bk  # 如果逆解失败则取此前记录的姿态角
        self.pose_list_servo = []  # pose_list_servo 在此置空，用于存放计算得到的一体化关节实际角度列表
        for j in range(len(self.pose_list)):
            self.pose_list_servo.append(self.model_to_servo(model_angle_list=self.pose_list[j])) # 将关节模型角度转换成一体化关节实际角度并添加进 pose_list_servo
        self.set_pose(tip_x_y=tip_x_ys[0], phi=phis[0], speed=10)
        self.pose_done()
        ...
        if t >= n * self.bit_time: # 轨迹要求的执行时间大于系统发送 n 个动作指令（所有轨迹点）所需的时间
            ...
            dr.set_angles(self.ID_list, self.pose_list_servo[0], 20, bit_wideth, 0) # 采用轨迹跟踪模式发送首条位置指令，根据轨迹跟踪模式要求，bit_wideth 需为实际指令发送带宽的一半
            while (time.time() - start) < (t / n): # 将轨迹执行时间按照点的数量均分
                time.sleep(0.001)
            bit_wideth1 = 1 / (time.time() - start) / 2 # 计算在 t>n 情况下的指令发送频率的一半
            for k in range(n):
                start = time.time()
                dr.set_angles(self.ID_list, self.pose_list_servo[k], 20, bit_wideth1, 0) # 采用轨迹跟踪模式，根据轨迹跟踪模式要求，bit_wideth1 需为实际指令发送带宽的一半
            ...
            for k in range(n):
                dr.set_angles(self.ID_list, self.pose_list_servo[k], 20, bit_wideth, 0) # 使用轨迹追踪模式控制机器人运动
    
    def set_poses_curve_pre(self, tip_x_ys=[], phis=[]):
        """预先计算机器人按顺序连续运动到多个指定位置和姿态所对应的一体化关节角度

        Args:
            tip_x_ys: 机器人末端连续多个坐标组成的列表[[x1, y1],[x2, y2],[x3, y3], ...]
            phi： 机器人末端连续多个姿态角组成的列表[pitch, roll, yaw]
        Returns:
            无
        Raises:
            pose_list_servo：一体化关节角度列表所组成的列表
        """
        ...
        if len(phis) > len(tip_x_ys):
            for ii in range(len(phis) - len(tip_x_ys)):
                tip_x_ys.append(tip_x_ys[len(tip_x_ys) -1]) # 如果输入的坐标值数目少于姿态角数目，则缺少的姿态角用最后一组坐标值补足
        n = len(tip_x_ys)
        self.tip_x_y_start = tip_x_ys[0].copy()
        self.phi_start = phis[0]
        self.clear_pose()
        for i in range(n):
            tip_x_y = tip_x_ys[i].copy() # 将轨迹点赋值给 tip_x_y
            phi = phis[i] # 将轨迹点赋值给 phi
            tip_x_y_bk = self.tip_x_y.copy()  # 记录此前末端坐标值，此处不能直接使用 tip_x_y_bk = self.tip_x_y 否则 tip_x_y_bk 会始终随着 self.tip_x_y 变化
            phi_bk = self.phi  # 记录此前末端姿态角
            self.tip_x_y = tip_x_y.copy()  # 记录此前末端坐标值
            self.phi = phi  # 记录此后末端姿态角
            if not self.save_pose(tip_x_y=tip_x_y, phi=phi, ud=1):  # 运动学逆解，并将求解结果存进 pose_list
                self.tip_x_y = tip_x_y_bk.copy()  # 如果逆解失败则取此前记录的末端坐标值
                self.phi = phi_bk  # 如果逆解失败则取此前记录的姿态角
    ...
    def set_poses_curve_start_point(self, speed=10):
        """控制机器人运动到轨迹中的首个位置和姿态, 用于set_arm_poses_cure_pre之后
        ...
        self.set_pose(tip_x_y=self.tip_x_y_start, phi=self.phi_start, speed=speed, acceleration=10)
        # dr.set_angles(self.ID_list, self.pose_list_servo[0], speed, 10, 1) # 控制机器人运动到轨迹中的首个位置和姿态
        self.pose_done() # 监控并等待动作执行结束

    def set_poses_curve_do(self, t=1):
        """控制机器人按顺序连续运动到多个指定位置和姿态, 与set_arm_poses_cure_pre连用
        ...
        for i in range(len(dr.cur_angle_list)):
            DETA_angle_list = list(
                map(lambda x: abs(x[0] - x[1]),
                    zip(self.pose_list_servo[0], dr.cur_angle_list)))
        if max(DETA_angle_list) > 1.0:  # 如果轨迹起点离当前位置比较远，则自动调用set_positions_curve_start_point(speed=10)，否则无需调用
            self.set_poses_curve_start_point(speed=10)
        n = len(self.pose_list_servo)
        if t >= n * self.bit_time: # 轨迹要求的执行时间大于系统发送 n 个动作指令（所有轨迹点）所需的时间
            print('t >= n * bit_time')  # 提示
            bit_wideth = n / t / 2 # 指令发送带宽的一半
    ...
    def tutorial_program(self):
        """在维持零力拖动的情况下记录机器人连续姿态信息，以实现示教编程
    ...
    def add_pose(self, t=0):
        ...
        if servo_list != False:
            self.pose_list.append(self.servo_to_model(servo_angle_list=servo_list)) # 将一体化关节角度转换成关节模型角度，并添加进 pose_list 中
    ...
    def do_pose(self, speed=10, acceleration=10, o_r=0, n=0):
        """依次执行机器人通过示教保存pose；
    ...
    def set_joints(self, angle_list=[0, 0, 0], speed=1.0, acceleration=10):
        ...
        Args:
            angle_list: 机器人驱动关节模型角度组成的列表[joint1, joint2, joint3]
            speed: 当前姿态被执行时转速最快关节的转动速度
            acceleration：关节加速度大小
        Returns:
            无
        ...
        if len(angle_list) == len(self.ID_list):
            pose = angle_list[:]
            self.seta = [math.pi / 180 * i for i in angle_list]
    ...
    def show_pose(self):
        ...
        print("当前机器人末端 x 坐标为: " + str(self.tip_x_y[0]))
        print("当前机器人末端 y 坐标为: " + str(self.tip_x_y[1]))
        print("当前机器人末端姿态角为: " + str(self.phi))
        return [self.tip_x_y[0], self.tip_x_y[1], self.phi]
    ...
    def impedance_control_joints(self, vel=1, tff=0, kp=5, kd=0.1):
        ...
        for j in range(len(self.ID_list)): # 提前 pose_list 中的最后一组关节模型角度列表（即机器人最后姿态），设置此姿态下关节阻抗系数
            dr.impedance_control(id_num=self.ID_list[j], pos=self.model_to_servo(self.pose_list[len(self.pose_list) - 1])[j],
                                 vel=vel, tff=tff, kp=kp, kd=kd)

    # 七、功能辅助函数
    def save_pose(self,  tip_x_y=[0, 0], phi=0, ud=1):
        """求解并保存当前机器人姿态下各个关节模型角度，并将角度制的角度列表添加进 pose_list

        Args:
            ud_temp: (up/down)用来选择反解中的何种解：
        ...
        seta_bk = self.seta.copy() # 记录此前的驱动关节模型角度（弧度制）
        pose = [] # 用于记录某个位姿下驱动关节模型角度（角度制）
        for num in range(self.ARM_NUMBER):
            x_y_phi_local = self.trans_coordinate(tip_x_y=tip_x_y, phi=phi, num=num+1) # 将动平台全局坐标系中的位姿变换到各支链坐标系
            # print(x_y_phi_local)
            tip_x_y_local = x_y_phi_local[:2] # 对应支链坐标系中的 x, y 坐标
            x_y_phi_local = x_y_phi_local[2] # 对应支链坐标系中的姿态角 phi
            if not self.inverse_kinematics(tip_x_y=tip_x_y_local, phi=x_y_phi_local, ud=ud): # 在对应支链中进行运动学逆解
                print("支链", num+1,"超出工作空间\n")
                self.seta = seta_bk.copy() # 记录内存中此前的驱动关节模型角度（弧度制）
                return False
            pose.append(self.theta[0] / math.pi * 180) # 记录对应支链的驱动关节模型角度（角度制）
        for i in range(len(pose)):
            if pose[i] < self.MIN_list[i]:
                print("第" + str(i + 1) + "个关节角度超出了最小极限角度")  # 检查关节模型角度是否小于最小允许值
                return False
            if pose[i] > self.MAX_list[i]:
                print("第" + str(i + 1) + "个关节角度超出了最大极限角度")  # 检查关节模型角度是否大于最大允许值
                return False
        self.seta = [DEG_RAD * i for i in pose].copy() # 更新内存中驱动关节模型角度（弧度制）
        self.pose_list.append(pose) # 记录目标位姿下驱动关节角度（角度制）
            ...
            if n == 0:
                if o_r == 0:
                    for i in range(len(self.pose_list)):
                        dr.set_angles(self.ID_list, self.model_to_servo(self.pose_list[i]), speed, acceleration, 1) # 依次执行 pose_list 中的pose（姿态）
                        if flg == 1:
                    ...
                    for j in range(len(self.pose_list)):
                        ...
                        dr.set_angles(self.ID_list, self.model_to_servo(self.pose_list[i]), speed, acceleration, 1) # 倒序执行 pose_list 中的pose（姿态）
                        if flg == 1:
                            self.pose_done()  # 等待并监控动作执行结束
                        time.sleep(self.tutorial_t_list[i])  # 设置每个姿态的停留时长
            else:
                dr.set_angles(self.ID_list, self.model_to_servo(self.pose_list[n-1]), speed, acceleration, 1)
                if flg == 1:
                    self.pose_done()  # 等待并监控动作执行结束
        ...
        for i in range(len(self.ID_list)):
            ...
            if servo != False:
                ...
                if servo != False:
                    ...
                    print("ID号为：" + str(self.ID_list[i]) + "的一体化关节读取角度失败！")
        if len(flag) != 0:
            return False
        return servo_list
        ...
        for i in range(len(self.ID_list)):
            ...
            if servo != False:
                ...
                if servo != False:
                    ...
                    flag.append(self.ID_list[i]) # 如果持续读取失败则记录读取失败的一体化关节编号
                    print("ID号为：" + str(self.ID_list[i]) + "的一体化关节取角度和速度失败！")
        if len(flag) != 0:
            return False
