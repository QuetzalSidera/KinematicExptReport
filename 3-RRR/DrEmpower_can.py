...
uart_baudrate = 115200 # 串口波特率，与CAN模块的串口波特率一致，（出厂默认为最高 460800）

# uart = serial.Serial('/dev/ttyAMA0', uart_baudrate)  # 在树莓派（raspbian）下控制一体化关节，相应的输入连接的串口
uart = serial.Serial('COM4', uart_baudrate)  # 在 windows 下控制一体化关节，相应的输入连接的COM口和波特率
...
# 多个关节阻抗控制
def impedance_control_multi(id_list=[1, 2, 3], angle_list=[0, 0, 0], speed_list=[0, 0, 0], tff_list=[0, 0, 0], kp_list=[0, 0, 0], kd_list=[0, 0, 0]):
    """多个一体化关节阻抗控制函数。
...
def get_state(id_num=0):
    ...
    [angle, speed]：当前关节角度和转速组成的列表。
        ...
        """
    try:
        return [read_property(id_num=id_num, property='axis.output_shaft.pos_estimate'), read_property(id_num=id_num, property='axis.output_shaft.vel_estimate')]
    except Exception as e:
